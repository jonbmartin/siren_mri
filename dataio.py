    ...
    def get_generalization_in_dict(self, spatial_img, img, idx):
        ...
        if self.generalization_mode == 'conv_cnp' or self.generalization_mode == 'conv_cnp_test':
            if self.test_sparsity == 'full':
                ...
                mask = torch.zeros_like(spatial_img)
                ny = mask.shape[1]
                samples = torch.randn(int(0.4*ny)) * int(ny/2.5) + int(ny/2) # std and mean specified 
                ...
                #                                                'img_sCS_cartesiaparse':img_sparse.cpu().numpy()})
            ...
            # in_dict = {'idx': torch.tensor(idx), 'coords': self.mgrid, 'img_sparse': img_sparse, 'ift_zfilled':ift_zfilled,'dc_mask':mask}
            in_dict = {'idx': torch.tensor(idx), 'coords': self.mgrid, 'img_sparse': img_sparse, 'dc_mask':mask}
