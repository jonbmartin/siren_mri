...
def main(rank, world_size, total_epochs, save_every, load_from_checkpoint_path, experiment_name, B):
    ...
    # fixed parameters
    print(B)
    batch_size = 12 # Was 16 for 128x128# with accumulation steps =16, this is an effective batch size of 96 (16*6)
    ...
    if config=='hyperoptII':
        ...
        fw_weight = 1e-5#1e-2 best for mse #1e-6#optim # 100 in paper
    ...
    if conv_encoder:
        if use_fourier_features:
            model = meta_modules.ConvolutionalNeuralProcessImplicit2DHypernetFourierFeatures(in_features=2*num_fourier_features,
                                                                    ...
                                                                    w0=w0, use_dc=True, hyper_dropout=dropout)
        else:
