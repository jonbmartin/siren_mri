...
def image_asinh(mask, model_output, gt):
    ...
    sio.savemat('evaluating_asinh_loss.mat',{'pred_no_tx':output_before_tx.cpu().numpy(), 'pred_tx':kspace_output_real.cpu().numpy(),
                                             'true_no_tx':gt_before_tx.cpu().numpy(), 'true_tx':kspace_gt_real.cpu().numpy()})
    sys.exit()
    # add a kspace domain loss:
    kspace_weight = 1/(128*128)
    kspace_loss = kspace_weight * ((kspace_output_real-kspace_gt_real)**2).sum()
